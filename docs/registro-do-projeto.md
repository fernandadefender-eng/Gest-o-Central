...
## 2026-09-19 (15) — Importações da EQUIPE (cadeados, TBG), relatórios em mais abas e auditoria de dependências
- **Cadeados/chaves dos postos** importados da aba oculta/congelada `CHAVES E SENHAS CADEADOS` (arquivo `02- EQUIPE-PR7- Validação de Julho 2025.xlsx`) → 29 `AcessoPosto`. A **senha do cadeado** fica em `segredo` (campo restrito/auditado, nunca na observação); trocas conhecidas (nova senha, cadeado trocado, "não temos a senha", outra empresa) viram entrada no `historico`. Script idempotente `apps/api/scripts/importar-cadeados.ts` (`--aplicar`).
- **Apoios TBG com valor acordado por pessoa** importados da aba `TBG-SEGURPRO` → config `ConfigSistema.tbg_apoios` (143 apoios, 86 com valor R$50–150). **Confirma a regra**: os R$350 são valor do **cliente**; o valor do **apoio** é o acordado por pessoa. Guarda também a regra da **camiseta branca**. O motor de valores (`pagamentos/valores-automaticos.ts` + helper `pagamentos/tbg.ts`) aplica esse valor quando o apoio atende um ponto TBG (casa por telefone e, na falta, por nome), sobrepondo o cálculo padrão. Script `importar-tbg-apoios.ts`; teste `scripts/teste-tbg.ts` (8).
- **Relatórios PDF/Excel no Roteirizador e no SAC** (gerador genérico `reports/relatorio-tabela-pdf.ts` — `gerarTabelaPdf`/`gerarTabelaXlsx`, reutilizável por qualquer aba): `GET /roteirizador/relatorio.{pdf,xlsx}` (Excel com despesas — uso interno) e `GET /sac/relatorio.{pdf,xlsx}` (SAC não tem valores). Botões "⬇ Excel"/"📄 PDF" nas abas SAC e Roteirizador (helper `baixarRelatorio` no painel). Smoke test `scripts/smoke-relatorios.ts` confirma que os 4 endpoints geram arquivo válido (magic %PDF / PK). Relatório da **equipe de monitoramento** segue pendente (conteúdo a definir com o usuário).
- **Segurança / dependências**: `npm audit` não tem correção segura disponível — todas as correções são *breaking* (NestJS 12 major, exceljs 3.4 downgrade). `npm audit fix` (sem `--force`) é no-op. Não aplicado `--force` para não arriscar travar a API/relatórios; aguardando o profissional passar o ponto específico da "vulnerabilidade de entrada e saída".
